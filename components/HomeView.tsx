...
export const HomeView: React.FC<Props> = ({ metrics, tracking, setView, navigateToChat }) => {
  ...
  return (
    <div className="pb-32 pt-12 px-5 font-sans">
      ...
      {/* Health Overview Card - Navigates to AI Chat */}
      <section 
      ...
      >
          ...
          <div className="text-[12px] text-slate-500 leading-relaxed mt-4 space-y-2 px-1">
            <p>请立即预约心血管专科医生，完善相关检查（24小时血压监测、心电图、肾功能、眼底检查等），可能需要药物降压治疗。</p>
            <p>通过积极的治疗和生活方式改变，血压是完全可以控制的，您的血糖正常是一个很好的基础。我们将一起制定可持续的健康计划。</p>
          </div>

          <div className="flex items-center justify-end space-x-1 text-[10px] text-blue-400 font-medium mt-4 italic">
             <Sparkles size={10} />
             <span>康康健康管家基于您的多维健康数据分析提供健康概览</span>
          </div>
      </section>
        ...
        <div className="grid grid-cols-2 gap-4">
            {metrics.map((m) => (
                <div 
                  ...
                  className="bg-white rounded-[1.8rem] p-4 shadow-sm border border-slate-50 flex flex-col relative overflow-hidden active:scale-[0.97] transition-all cursor-pointer"
                >
                    ...
                    <button className="text-[11px] font-bold text-blue-600 bg-blue-50 py-1.5 rounded-xl mb-3">开始测量</button>
            ...
            {tracking.map(t => (
                <div key={t.id} className="bg-white rounded-[1.8rem] p-5 shadow-sm border border-slate-50 flex items-center space-x-4">
                    <div className={`w-14 h-14 rounded-2xl bg-slate-50 flex items-center justify-center text-xl shadow-inner`}>
                        {t.id === '1' && <img src="https://img.icons8.com/ios-filled/50/000000/dumbbell.png" className="w-8 h-8 opacity-70" />}
                        {t.id === '2' && <img src="https://img.icons8.com/ios-filled/50/000000/walking.png" className="w-8 h-8 opacity-70" />}
                        {t.id === '3' && <img src="https://img.icons8.com/ios-filled/50/000000/apple.png" className="w-8 h-8 opacity-70" />}
                        {t.id === '4' && <img src="https://img.icons8.com/ios-filled/50/000000/bed.png" className="w-8 h-8 opacity-70" />}
                        {t.id === '5' && <img src="https://img.icons8.com/ios-filled/50/000000/water.png" className="w-8 h-8 opacity-70" />}
                    </div>
